...
using namespace read_joint_dispatcher;
...
bool Task::configureHook()
{
    ...
    { // Create the outputs
	    ...
	    OutputPort *port;
            if (!(port = dynamic_cast<OutputPort*>(getPort(conf.name))))
            {
                LOG_ERROR("output port %s is not available", conf.name.c_str());
                clearPorts();
                return false;
            }
    ...
    { // Now create the inputs *and* the dispatches at the same time
	    ...
	    InputPort* port;
            if (!(port = dynamic_cast<InputPort*>(getPort(conf.input))))
            {
